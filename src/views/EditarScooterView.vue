<template>
  <div class="editar-scooter-view">
    <div class="header">
      <h1>Editar Scooter</h1>
    </div>
...
onMounted(async () => {
  try {
    ...
    if (response.data) {
      const [street, neighborhood, city, district] = response.data.address.split(',').map(part => part.trim());
      form.value = {
    ...
    console.error('Error al cargar los datos del scooter:', error);
    alert('No se pudo cargar la información del scooter.');
...
const handleSubmit = async () => {
  try {
    ...
    await api.put(`/Scooter/${scooterId}`, scooterData);
    alert('Scooter actualizado exitosamente.');
    router.push('/mis-scooters');
  } catch (error) {
    console.error('Error al actualizar el scooter:', error);
    alert('Ocurrió un error al actualizar el scooter.');
  }
};
